...
try{
 ...
 await context.addInitScript(()=>{if(!localStorage.getItem('hamava.web.key'))localStorage.setItem('hamava.web.key','AQ.Ab8o-test-only-fake-auth-key-2026');if(!localStorage.getItem('hamava.web.settings'))localStorage.setItem('hamava.web.settings',JSON.stringify({mode:'both',dubVolume:0}));window.__hamavaBufferStarts=0;window.__hamavaPlaybackRates=[];window.__hamavaBufferDurations=[];const start=AudioBufferSourceNode.prototype.start;AudioBufferSourceNode.prototype.start=function(...args){window.__hamavaBufferStarts++;window.__hamavaPlaybackRates.push(this.playbackRate.value);window.__hamavaBufferDurations.push(this.buffer?.duration);return start.apply(this,args);};window.YT={Player:class{constructor(_node,options){this.options=options;this.state=2;this.time=0;window.__ytPlayer=this;setTimeout(()=>options.events.onReady(),0);}playVideo(){this.state=1;this.options.events.onStateChange({data:1});}pauseVideo(){this.state=2;this.options.events.onStateChange({data:2});}seekTo(time){this.time=time;this.state=2;}setVolume(value){this.volume=value;}getCurrentTime(){return this.time;}getDuration(){return 3;}getPlayerState(){return this.state;}destroy(){}getVideoData(){return{title:'YouTube playback test'};}}};});
 await context.route('https://generativelanguage.googleapis.com/**',async route=>{
   ...
   const isYoutube=JSON.stringify(body).includes('"type":"video"');
   const content=body.model.includes('tts')?[{type:'audio',mime_type:'audio/l16',sample_rate:24000,channels:1,data:pcm}]:[{type:'text',text:JSON.stringify({complete:true,cues:[{start:0,end:isYoutube?1.5:3,source:'Hello world',translation:'سلام دنیا',speaker:'a',voice:'female'}]})}];
 ...
 await page.goto(`http://127.0.0.1:${server.address().port}/HamAva/`);await page.waitForSelector('.key-ready');await page.getByText('نسخه ۱.۱.۰',{exact:false}).first().waitFor();assert.equal(await page.getByRole('link',{name:'کد منبع هم‌آوا در GitHub'}).getAttribute('href'),'https://github.com/mostafa5804/HamAva');
 await page.getByRole('button',{name:'تنظیمات'}).click();await page.getByRole('button',{name:'بررسی مدل‌های API Key'}).click();await page.getByText(/مدل دریافت شد/).waitFor();assert.ok(await page.locator('#ttsModel-picker option[value="gemini-3.8-flash-lite-tts"]').count());await page.locator('#ttsModel-picker').selectOption('gemini-3.1-flash-tts-preview');await page.waitForFunction(()=>JSON.parse(localStorage.getItem('hamava.web.settings')).ttsModel==='gemini-3.1-flash-tts-preview');console.log('PASS API model list is selectable and selection is saved');await page.getByRole('button',{name:'بستن تنظیمات'}).click();
 ...
 await page.locator('.desktop-translation-controls button').filter({hasText:'شروع ترجمه'}).click();await page.locator('.desktop-translation-controls button').filter({hasText:'پخش با ترجمه'}).waitFor();const youtubeStarts=await page.evaluate(()=>window.__hamavaBufferStarts),rateStart=await page.evaluate(()=>window.__hamavaPlaybackRates.length),durationStart=await page.evaluate(()=>window.__hamavaBufferDurations.length);await page.locator('.desktop-translation-controls button').filter({hasText:'پخش با ترجمه'}).click();await page.waitForFunction(before=>window.__hamavaBufferStarts>before,youtubeStarts);assert.equal(await page.locator('.error-message').count(),0);assert.ok(await page.evaluate(index=>window.__hamavaPlaybackRates.slice(index).length>0&&window.__hamavaPlaybackRates.slice(index).every(rate=>rate===1),rateStart),'dubbing must play at 1x');assert.ok(Math.abs(await page.evaluate(index=>window.__hamavaBufferDurations[index]-3,durationStart))<.05,'a 3-second TTS utterance must not be squeezed into a 1.5-second subtitle cue');console.log('PASS YouTube dubbing keeps its natural audio duration at 1x');
 await page.evaluate(async()=>{for(let i=0;i<4;i++){window.__ytPlayer.time+=.24;await new Promise(resolve=>setTimeout(resolve,120));}});assert.equal(await page.evaluate(()=>window.__hamavaBufferStarts)-youtubeStarts,1,'jittery YouTube time reports must not restart and skip the playing dub');console.log('PASS YouTube clock jitter does not interrupt an audio clip');
 ...
 const beforeVideo=calls;await page.getByRole('combobox',{name:'محتوای خروجی'}).click();assert.equal(await page.getByRole('option',{name:'فقط دوبله'}).count(),1);assert.equal(await page.getByRole('option',{name:'دوبله و زیرنویس'}).count(),1);await page.getByRole('option',{name:'فقط زیرنویس'}).click();await page.getByRole('button',{name:'ساخت ویدیوی خروجی',exact:true}).click();
 await page.getByRole('link',{name:'دانلود ویدیوی خروجی',exact:true}).waitFor({timeout:15000});assert.equal(calls,beforeVideo);
 const vwait=page.waitForEvent('download');await page.getByRole('link',{name:'دانلود ویدیوی خروجی',exact:true}).click();await (await vwait).saveAs(root+'/qa-web-video.webm');assert.ok(fs.statSync(root+'/qa-web-video.webm').size>1000);const bottomBand=execFileSync('ffmpeg',['-hide_banner','-loglevel','info','-ss','0.5','-i',root+'/qa-web-video.webm','-vf','crop=iw*.4:ih*.5:iw*.3:ih*.5,signalstats,metadata=print:file=-','-frames:v','1','-f','null','-'],{encoding:'utf8',stdio:['ignore','pipe','pipe']});const brightness=Number(/lavfi\.signalstats\.YAVG=([\d.]+)/.exec(bottomBand)?.[1]);assert.ok(Number.isFinite(brightness)&&brightness<35,'subtitle text and background must be burned into the exported video');console.log('PASS selectable subtitle-only video export burns captions without another API call');
 await page.evaluate(()=>{
