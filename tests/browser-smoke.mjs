...
try{
 ...
 await context.addInitScript(()=>{if(!localStorage.getItem('hamava.web.key'))localStorage.setItem('hamava.web.key','AQ.Ab8o-test-only-fake-auth-key-2026');if(!localStorage.getItem('hamava.web.settings'))localStorage.setItem('hamava.web.settings',JSON.stringify({mode:'both',dubVolume:0}));window.__hamavaBufferStarts=0;window.__hamavaPlaybackRates=[];const start=AudioBufferSourceNode.prototype.start;AudioBufferSourceNode.prototype.start=function(...args){window.__hamavaBufferStarts++;window.__hamavaPlaybackRates.push(this.playbackRate.value);return start.apply(this,args);};window.YT={Player:class{constructor(_node,options){this.options=options;this.state=2;this.time=0;window.__ytPlayer=this;setTimeout(()=>options.events.onReady(),0);}playVideo(){this.state=1;this.options.events.onStateChange({data:1});}pauseVideo(){this.state=2;this.options.events.onStateChange({data:2});}seekTo(time){this.time=time;this.state=2;}setVolume(value){this.volume=value;}getCurrentTime(){return this.time;}getDuration(){return 3;}getPlayerState(){return this.state;}destroy(){}getVideoData(){return{title:'YouTube playback test'};}}};});
 ...
 await page.goto(`http://127.0.0.1:${server.address().port}/HamAva/`);await page.waitForSelector('.key-ready');await page.getByText('نسخه ۱.۰.۱۰',{exact:false}).first().waitFor();assert.equal(await page.getByRole('link',{name:'کد منبع هم‌آوا در GitHub'}).getAttribute('href'),'https://github.com/mostafa5804/HamAva');
 ...
 await page.locator('.desktop-translation-controls button').filter({hasText:'شروع ترجمه'}).click();await page.locator('.desktop-translation-controls button').filter({hasText:'پخش با ترجمه'}).waitFor();const youtubeStarts=await page.evaluate(()=>window.__hamavaBufferStarts),rateStart=await page.evaluate(()=>window.__hamavaPlaybackRates.length);await page.locator('.desktop-translation-controls button').filter({hasText:'پخش با ترجمه'}).click();await page.waitForFunction(before=>window.__hamavaBufferStarts>before,youtubeStarts);assert.equal(await page.locator('.error-message').count(),0);assert.ok(await page.evaluate(index=>window.__hamavaPlaybackRates.slice(index).length>0&&window.__hamavaPlaybackRates.slice(index).every(rate=>rate===1),rateStart),'stretched audio must play at 1x to preserve pitch');console.log('PASS YouTube dub playback is synchronized at natural pitch for an overlong generated clip');
