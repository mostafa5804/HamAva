...
import { defaults, sanitizeSettings, youtubeId, directUrl, errorText, formatTime, type Settings, type SourceKind, type Cue, type AudioClip, type Mode } from '@/lib/hamava/types';
...
type Media = { kind:SourceKind; url:string; name:string; id?:string; revision:number };
...
export default function Studio(){
  ...
  const [key,setKey]=useState(storedKey),[keyDraft,setKeyDraft]=useState(storedKey),[remember,setRemember]=useState(true),[showKey,setShowKey]=useState(false),[sheet,setSheet]=useState(false),[keyNote,setKeyNote]=useState('');
  ...
  const ensureEngine=useCallback(()=>engine.current||(engine.current=new MediaEngine(settingsRef.current,report,setTalking)),[report]);
  const caption=media?.kind==='youtube'?cues.find(c=>time>=c.start+settings.captionOffset&&time<c.end+settings.captionOffset)||null:liveCaption;
  ...
  useEffect(()=>{
    ...
    return()=>{window.removeEventListener('beforeinstallprompt',prompt);abort.current?.abort();if(recorder.current?.state==='recording')recorder.current.stop();engine.current?.dispose();yt.current?.destroy();if(blob.current)URL.revokeObjectURL(blob.current);if(recordedUrlRef.current)URL.revokeObjectURL(recordedUrlRef.current);clipsRef.current.forEach(c=>URL.revokeObjectURL(c.url));clearTimeout(captionTimer.current);};
  },[]);
  useEffect(()=>{try{localStorage.setItem('hamava.web.settings',JSON.stringify(settings));}catch{}engine.current?.apply(settings,talking);if(video.current&&!engine.current?.source)video.current.volume=settings.originalVolume/100;yt.current?.setVolume(settings.originalVolume*(settings.duck&&talking&&settings.mode!=='subtitle'?.2:1));},[settings,talking]);
  useEffect(()=>{
    const hidden=()=>{if(!document.hidden)return;if(liveEnabled.current){engine.current?.stopLive();setStatus('paused');}video.current?.pause();yt.current?.pauseVideo();engine.current?.clearSound();setPlaying(false);};
  ...
  function selectFile(f:File|undefined){if(!f)return;if(!f.type.startsWith('video/')&&!f.type.startsWith('audio/')&&!/\.(mp4|webm|mov|m4v|mp3|m4a|wav|ogg)$/i.test(f.name)){setError('یک فایل ویدیویی یا صوتی قابل پخش انتخاب کن.');return;}resetMedia();blob.current=URL.createObjectURL(f);sourceFile.current=f;mediaProbe.current=probeMediaFile(f);setMedia({kind:'file',url:blob.current,name:f.name,revision:epoch.current});}
  function loadLink(){try{const input=tab==='youtube'?ytUrl:url;if(tab==='youtube'){const id=youtubeId(input);resetMedia();sourceFile.current=null;mediaProbe.current=null;setMedia({kind:'youtube',url:`https://www.youtube.com/watch?v=${id}`,id,name:'ویدیوی یوتیوب',revision:epoch.current});}else{const link=directUrl(input);resetMedia();sourceFile.current=null;mediaProbe.current=probeMediaFile({name:decodeURIComponent(new URL(link).pathname.split('/').pop()||'media')});setMedia({kind:'url',url:link,name:decodeURIComponent(new URL(link).pathname.split('/').pop()||'ویدیوی آنلاین'),revision:epoch.current});}}catch(e){report(e);}}
  ...
  useEffect(()=>{
    ...
    void loadYoutube().then(api=>{
      ...
      player=new api.Player(node,{videoId:media.id,width:'100%',height:'100%',playerVars:{playsinline:1,origin:location.origin,rel:0,fs:0},events:{
        onReady:()=>{if(cancelled)return;yt.current=player!;setMediaReady(true);player!.setVolume(settingsRef.current.originalVolume);setDuration(player!.getDuration()||0);const title=player!.getVideoData().title;if(title)setMedia(m=>m&&m.id===media.id?{...m,name:title}:m);},
  ...
  async function prepare(){
    ...
    yt.current?.pauseVideo();engine.current?.clearSound();setPlaying(false);setError('');setNotice('');setPreparing(true);setPartialReady(false);setStatus('preparing');setProgress(0);
    ...
    try{
      ...
      const addClip=(clip:AudioClip)=>{clipsRef.current=[...clipsRef.current,clip].sort((a,b)=>a.start-b.start);setClips(list=>[...list,clip].sort((a,b)=>a.start-b.start));setPartialReady(true);};
      ...
      if(epoch.current!==thisEpoch)return;setProgress(100);setStatus('ready');setNotice('ترجمه آماده است. پخش را بزن و این صفحه را باز نگه دار؛ با بازنشانی صفحه، ترجمه آماده‌شده پاک می‌شود.');yt.current?.seekTo(0,true);
  ...
  function onPause(){setPlaying(false);engine.current?.clearSound();if(liveEnabled.current){engine.current?.stopLive();setStatus('paused');}}
  ...
  function saveKey(){
    const clean=keyDraft.trim();if(clean&&!/^[A-Za-z0-9_-]{20,200}$/.test(clean)){setKeyNote('فرمت کلید درست نیست؛ خود کلید را بدون فاصله وارد کن.');return;}
  ...
  function stopRecording(){if(recorder.current?.state==='recording'){video.current?.pause();stopLive();recorder.current.stop();}}
  async function recordDubbedVideo(){
    if(!media||media.kind==='youtube'||!video.current){setNotice('خروجی ویدیویی برای فایل یا لینک مستقیم قابل ساخت است؛ یوتیوب به‌دلیل iframe خام قابل ضبط نیست.');return;}
    if(settings.mode==='subtitle'){setNotice('برای ساخت خروجی دوبله، حالت «هر دو» یا «دوبله» را انتخاب کن.');return;}
    if(!keyRef.current.trim()){setSheet(true);setKeyNote('برای ساخت خروجی دوبله، کلید Gemini را وارد و ذخیره کن.');return;}
    try{
      ...
      if(!capture||!capture.getVideoTracks().length)throw new Error('این مرورگر ضبط مستقیم ویدیو را پشتیبانی نمی‌کند. Chrome یا Edge را امتحان کن.');
      const stream=new MediaStream(capture.getVideoTracks().map(track=>track.clone()));
      ...
      r.ondataavailable=e=>{if(e.data.size)recordedChunks.current.push(e.data);};
      r.onerror=()=>{stream.getTracks().forEach(track=>track.stop());setRecording(false);setError('ساخت فایل ویدیویی دوبله‌شده متوقف شد.');};
      r.onstop=()=>{const output=new Blob(recordedChunks.current,{type:mime});stream.getTracks().forEach(track=>track.stop());const u=URL.createObjectURL(output);recordedUrlRef.current=u;setRecordedUrl(u);setRecording(false);setNotice('فایل ویدیوی دوبله‌شده آماده‌ی دانلود است.');recorder.current=null;};
      video.current.currentTime=0;setTime(0);setRecording(true);r.start(1000);await beginLive();
      if(!liveEnabled.current){r.stop();throw new Error('ترجمه زنده برای ضبط خروجی فعال نشد.');}
      await video.current.play();
    }catch(e){setRecording(false);report(e);}
  }
  function download(){const content=exportCaptions(cues,{format:'srt',bilingual:settings.bilingual,offset:settings.captionOffset});const u=URL.createObjectURL(new Blob(['\ufeff'+content],{type:'text/plain;charset=utf-8'}));const a=document.createElement('a');a.href=u;a.download=downloadName(mediaRef.current?.name||'HamAva','زیرنویس فارسی','srt');a.click();setTimeout(()=>URL.revokeObjectURL(u),1500);}
  /**
  ...
  const translationControls={
    mode:settings.mode,onMode:(mode:Mode)=>update({mode}),preparing,progress,progressLabel,
    disabled:!mediaReady,
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      <section className="source-panel card" aria-labelledby="source-title"><div className="section-heading"><h2 id="source-title">چی تماشا می‌کنی؟</h2><span className="language-badge">فارسی <span>FA</span></span></div>
        <Tabs dir="rtl" value={tab} onValueChange={v=>setTab(v as SourceKind)}><TabsList className="source-tabs"><TabsTrigger value="file"><Upload/>فایل من</TabsTrigger><TabsTrigger value="url"><Link2/>لینک مستقیم</TabsTrigger><TabsTrigger value="youtube"><Youtube/>یوتیوب</TabsTrigger></TabsList>
          <TabsContent value="file"><div className="file-drop" onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();if(!preparing)selectFile(e.dataTransfer.files[0]);}}><input ref={file} type="file" accept="video/*,audio/*,.mkv,.mov" aria-label="انتخاب ویدیو از گوشی" onChange={e=>{selectFile(e.target.files?.[0]);e.target.value='';}} disabled={preparing} className="sr-only"/><div className="upload-icon"><Upload size={22}/></div><div><strong>ویدیو را از گوشی انتخاب کن</strong><p>فایل در همین دستگاه پخش می‌شود</p></div><button className="outline-button" disabled={preparing} onClick={()=>file.current?.click()}>انتخاب فایل<ChevronLeft size={16}/></button></div></TabsContent>
          <TabsContent value="url"><form className="link-form" onSubmit={e=>{e.preventDefault();loadLink();}}><label className="sr-only" htmlFor="direct-link">لینک مستقیم ویدیو</label><input id="direct-link" dir="ltr" type="url" placeholder="https://example.com/video.mp4" value={url} onChange={e=>setUrl(e.target.value)} required/><button className="outline-button" disabled={preparing}>باز کردن</button></form><p className="hint">لینک خود فایل، مثل MP4 یا WebM؛ میزبان باید دسترسی صوتی مرورگر را مجاز کرده باشد.</p></TabsContent>
          <TabsContent value="youtube"><form className="link-form" onSubmit={e=>{e.preventDefault();loadLink();}}><label className="sr-only" htmlFor="youtube-link">لینک ویدیوی یوتیوب</label><input id="youtube-link" dir="ltr" type="url" placeholder="https://youtube.com/watch?v=..." value={ytUrl} onChange={e=>setYtUrl(e.target.value)} required/><button className="outline-button" disabled={preparing}>باز کردن</button></form><p className="hint">ویدیوی عمومی: اول ترجمه و دوبله آماده می‌شود، بعد پخش می‌کنی.</p></TabsContent>
        </Tabs>
      </section>

      <section className="viewing-column">
        <div className="player-card"><div className="stage" ref={stage}>
          <video ref={video} src={!youtube?media?.url:undefined} crossOrigin="anonymous" playsInline preload="metadata" className={media&&!youtube?'':'hidden'} onLoadedMetadata={()=>{setDuration(video.current?.duration||0);setMediaReady(true);if(video.current&&!engine.current?.source)video.current.volume=settingsRef.current.originalVolume/100;}} onTimeUpdate={()=>{if(!dragging.current)setTime(video.current?.currentTime||0);const v=video.current;if(v)engine.current?.syncPrepared(clipsRef.current,v.currentTime,!v.paused);}} onPlay={()=>{setPlaying(true);if(liveEnabled.current&&!engine.current?.active)void beginLive();}} onPause={onPause} onSeeking={onSeek} onSeeked={onSeeked} onEnded={()=>{setPlaying(false);if(recorder.current?.state==='recording')recorder.current.stop();stopLive();}} onError={()=>{if(mediaRef.current?.kind!=='youtube'&&mediaRef.current){setMediaReady(false);stopLive();setError(mediaRef.current.kind==='url'?'ویدیو باز نشد. لینک باید مستقیم، قابل پخش و دارای مجوز CORS باشد. می‌توانی فایل را دانلود و از بخش فایل وارد کنی.':'این فرمت در مرورگر قابل پخش نیست. نسخه MP4 با کدک H.264 یا WebM را امتحان کن.');}}}/>
        ...
        <section className="transcript card"><div className="section-heading"><h2><Subtitles size={19}/>متن ترجمه</h2><button className="text-button" disabled={!cues.length} onClick={download}><Download size={16}/>دریافت SRT</button></div>{cues.length?<div className="cue-list">{cues.map((c,i)=><button key={`${c.start}-${i}`} className={`cue-row ${caption===c||caption?.start===c.start?'current':''}`} onClick={()=>seek(c.start+settings.captionOffset)}><time dir="ltr">{formatTime(c.start)}</time><span>{c.translation}{settings.bilingual&&<small dir="auto">{c.source}</small>}</span></button>)}</div>:<div className="transcript-empty"><span className="caption-mark">Aa</span><p>ترجمه‌ها اینجا ظاهر می‌شوند<span>زیرنویس فارسی را هنگام تماشا دنبال کن</span></p></div>}</section>
      </section>
      ...
      <aside className="controls-column">
        <section className="translation-card card"><div className="section-heading"><h2><AudioLines size={20}/>ترجمه و دوبله</h2><span className="mini-label">GEMINI</span></div>
          {youtube?<><Choice label="صدای دوبله" value={settings.voice} disabled={preparing} onChange={changeVoice} items={[["auto","خودکار · متناسب با گوینده"],["female","صدای زن · Kore"],["male","صدای مرد · Charon"]]}/><p className="hint voice-hint">صداهای آماده زن و مرد؛ انتخاب خودکار ممکن است خطا کند. صدای اصلی کپی نمی‌شود. سرعت دوبله برای هماهنگی با تصویر تنظیم می‌شود.</p></>:<div className="voice-summary"><div className="voice-symbol"><Mic2 size={20}/></div><div><strong>لحن گوینده، به فارسی</strong><p>تطبیق خودکار صدا با تحلیل مدل</p></div></div>}
    ...
    <footer className="app-footer"><span>هم‌آوا <b>·</b> نسخه ۱.۴</span><span>بدون سقف داخلی؛ هزینه و سهمیه تابع Gemini است.</span></footer>
    ...
    <Sheet open={sheet} onOpenChange={setSheet}><SheetContent side="left" showCloseButton={false} className="settings-sheet" dir="rtl"><SheetHeader><div className="section-heading"><SheetTitle>تنظیمات هم‌آوا</SheetTitle><SheetClose className="icon-button" aria-label="بستن تنظیمات"><X size={21}/></SheetClose></div><SheetDescription>تنظیمات مخصوص همین مرورگر</SheetDescription></SheetHeader>
      <div className="settings-scroll"><section><h3><KeyRound size={19}/>کلید Gemini</h3><label htmlFor="api-key">کلید API شخصی</label><div className="key-input"><input id="api-key" dir="ltr" type={showKey?'text':'password'} autoComplete="off" spellCheck={false} placeholder="AIza…" value={keyDraft} onChange={e=>{setKeyDraft(e.target.value);setKeyNote('');}}/><button className="icon-button" onClick={()=>setShowKey(v=>!v)} aria-label={showKey?'پنهان کردن کلید':'نمایش کلید'}>{showKey?<EyeOff size={19}/>:<Eye size={19}/>}</button></div><Toggle label="ذخیره کلید در این مرورگر" checked={remember} onChange={v=>{setRemember(v);if(!v){try{localStorage.removeItem('hamava.web.key');}catch{}}}}/><div className="key-buttons"><button className="primary-button" disabled={preparing} onClick={saveKey}><Check size={17}/>ذخیره کلید</button><button className="outline-button" disabled={preparing||!key} onClick={()=>{setKey('');setKeyDraft('');stopLive();try{localStorage.removeItem('hamava.web.key');}catch{}setKeyNote('کلید حذف شد.');}}>حذف کلید</button></div>{keyNote&&<p className="key-note" role="status">{keyNote}</p>}<p className="hint">کلید به سرور هم‌آوا فرستاده نمی‌شود؛ برای ترجمه، کلید و صدای ویدیو یا لینک یوتیوب مستقیم به گوگل ارسال می‌شوند. ذخیره مرورگر رمزنگاری جداگانه ندارد.</p><a className="inline-link" href="https://aistudio.google.com/apikey" target="_blank" rel="noreferrer">دریافت کلید از Google AI Studio <ChevronLeft size={14}/></a></section>
        <section><h3><Subtitles size={19}/>نمایش زیرنویس</h3><div className="caption-preview"><span style={{fontSize:settings.captionSize,color:settings.captionColor,backgroundColor:`rgba(3,12,17,${settings.captionOpacity/100})`}}>دنیا را به زبان خودت ببین.{settings.bilingual&&<small>See the world in your language.</small>}</span></div><Range label="اندازه نوشته" value={settings.captionSize} min={16} max={40} suffix="" onChange={v=>update({captionSize:v})}/><Range label="تیرگی پس‌زمینه" value={settings.captionOpacity} onChange={v=>update({captionOpacity:v})}/><Choice label="جای زیرنویس" value={settings.captionPosition} onChange={v=>update({captionPosition:v as Settings['captionPosition']})} items={[["bottom","پایین تصویر"],["top","بالای تصویر"]]}/><Choice label="رنگ نوشته" value={settings.captionColor} onChange={v=>update({captionColor:v})} items={[["#ffffff","سفید"],["#ffe59a","زرد روشن"],["#9df0da","سبز روشن"]]}/><Toggle label="نمایش متن اصلی کنار فارسی" checked={settings.bilingual} onChange={v=>update({bilingual:v})}/><Range label="تأخیر زیرنویس یوتیوب" value={settings.captionOffset} min={-10} max={10} step={.25} suffix=" ثانیه" onChange={v=>update({captionOffset:v})}/></section>
        <section><h3><Settings2 size={19}/>مدل‌های Gemini</h3><p className="hint">فایل، لینک مستقیم و یوتیوب با «تحلیل ویدیو و فایل» ترجمه می‌شوند و زمان‌بندی دقیق می‌گیرند؛ «ترجمه زنده» فقط برای فایل‌های بزرگ‌تر از سقف تحلیل به‌کار می‌رود. ساخت صدا با TTS است. تغییر مدل، ترجمه فعال را متوقف می‌کند.</p>{([['model','ترجمه زنده (فایل‌های بزرگ)'],['videoModel','تحلیل ویدیو و فایل'],['ttsModel','ساخت صدای فارسی']] as const).map(([field,label])=><div className="model-field" key={field}><label htmlFor={field}>{label}</label><input id={field} dir="ltr" defaultValue={settings[field]} key={settings[field]} disabled={preparing} onBlur={e=>{if(e.target.value!==settings[field]){if(!/^[a-z0-9.-]{4,100}$/.test(e.target.value)){e.target.value=settings[field];return;}stopLive();update({[field]:e.target.value});if(field!=='model'){clearPrepared();setCues([]);setStatus('idle');}}}}/></div>)}<p className="hint">حفظ لحن در ترجمه همزمان تقریبی است و ممکن است بین گوینده‌ها تغییر کند.</p></section>
