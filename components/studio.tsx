...
export default function Studio(){
  ...
  useEffect(()=>{
    ...
    const timer=setTimeout(()=>{
      archiveWrites.current=archiveWrites.current.catch(()=>{}).then(async()=>{
        if(cancelled)return;
        const savedClips=await Promise.all(clips.map(async({url,blob:clipBlob,...clip})=>({...clip,blob:clipBlob||await (await fetch(url)).blob()})));
        if(cancelled)return;
  ...
  async function downloadDubAudio(){
    ...
    try{
      ...
      try{
        const {blob}=await renderDubbingTrack({clips:clipsRef.current.map(c=>({start:c.start,end:c.end,url:c.url,blob:c.blob})),duration:duration||cues.at(-1)?.end||0,decodeAudio:b=>ctx.decodeAudioData(b)});
